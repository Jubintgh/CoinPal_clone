...
## Features
    * User Authentication / Session: User's can sign up/ log in with specific credentials and each logged in user is protected by establishing a seperate session.

    User Wallet: Each authorized User is provided with it's own Crypto wallet that includes the 3 cryptocurrencies at the time of signup

    Crypto Transactions: Each authorized User is able to send, request, cancel or reject crypto transactions that it has made to other authorized users or other users have made to it.

    Send Friend Requests: Each authorized User is able to send or recieve Friend requests to/from other authorized users
